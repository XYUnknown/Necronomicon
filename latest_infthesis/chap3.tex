...
However, comparing to monolithic systems, distributed systems are complex and challenging to design, implement, test and debug due to the necessity for coordination, synchronisation, and communication among distributed components. In addition, it is a common practice that, to implement a system, a monolithic design is initially adopted since the implementation and deployment of a monolithic system is straightforward. Such a monolithic system needs to be re-structured and migrated into a distributed design when it needs to be expanded into a larger scale and it requires non-trivial effort to be put into the migration of a monolithic system into a distributed design.

To address these issues in the design and implementation of a distributed system as well as migrating a monolithic systems into a distributed setting, we propose our design of a UMI library in Rust.
