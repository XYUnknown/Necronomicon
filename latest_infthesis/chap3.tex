...
\begin{gather*}
    \forall t \in \mathcal{T}.\; \mathcal{D}, [(n, t)] \leadsto \mathcal{D'}, [(n, v)] \land |\mathcal{D}| = \mathcal{S} \\ \Rightarrow \\ \mathcal{S},  t|_@ \longrightarrow \mathcal{S'}, v' \land |\mathcal{D}'| = \mathcal{S}' \land v|_@ = v'
\end{gather*}
\begin{highlightnew}Note that the notation $[(n, v)]$ means a singleton configuration stack.\end{highlightnew} In addition, $|\cdot|$ and $\cdot|_@$ are operators that erase addresses of nodes from distributed program states and terms defined as:
\begin{align*}
...
\[|\mathcal{D}| = S \land \mathscr{l} \notin \textbf{dom}\;\mathcal{D}(n) \Rightarrow \mathscr{l} \notin \textbf{dom}\;\mathcal{S}\]
\end{lemma}
\end{highlightnew}
...
Given a distributed program state $\mathcal{D}$ and a monolithic program state $\mathcal{S}$, $n$ is a node in the domain of $\mathcal{D}$:
\[|\mathcal{D}| = S \land \mathscr{l} \in \textbf{dom}\;\mathcal{D}(n) \Rightarrow \mathscr{l} \in \textbf{dom}\;\mathcal{S}\]
\end{lemma}
\end{highlightnew}
