...
\begin{figure}
    \begin{mathpar}
        ...
        {\mathcal{D}, \mathcal{C} \concat (n, t[x/?]) \concat (n', \texttt{let mut } x = v; x) \longrightarrow \\\mathcal{D} \otimes (n' \mapsto \mathcal{S}\otimes \mathscr{l} \mapsto (v, k)), \mathcal{C} \concat (n, t[x/ \mathscr{l}^\bullet@n'])}
...
\end{definition}
\begin{definition}[Environment Strengthening] Let $\Gamma_1$ and $\Gamma_2$ be typing environments. We say that $\Gamma_1$ strengthens $\Gamma_2$, denoted $\Gamma_1 \sqsubseteq \Gamma_2$, if and only if $\mathrm{dom}(\Gamma_1) = \mathrm{dom}(\Gamma_2)$ and for all $x \in \mathrm{dom}(\Gamma_1)$ where $\Gamma_1(x) = \langle \widetilde{T_1} \rangle^1$, we have $\Gamma_2(x) = \langle \widetilde{T_2} \rangle^1$ where $\widetilde{T_1} \sqsubseteq \widetilde{T_2}$.
\end{definition}
...
\inferrule*[right=(T-LvBor)]{
  ...
  \Gamma \vdash *w : \langle \sqcup_i T_i \rangle^{\sqcap_i m_i}
}
\end{mathpar}
...
\begin{definition}[Type Containment] Let $\Gamma$ be an environment where $\Gamma(x) = \langle \widetilde{T} \rangle^1$ for some l. Then $\Gamma \vdash x \leadsto T_y$ denotes that variable $x$ contains type $T_y$ and is defined as $\emph{\texttt{contains}}(\Gamma, \widetilde{T}, T_y)$ where:
\[
...
\begin{definition}[Move] Let $\Gamma$ be an environment where $\Gamma(x) = \langle \widetilde{T_1} \rangle^1$ for some lifetime 1, and $w$ an lval where $w \triangleq \pi_x \mid x$. Then $\emph{\texttt{move}}(\Gamma, w)$ is a partial function defined as $\Gamma[x \mapsto \langle \widetilde{T_2} \rangle^1]$ where $\widetilde{T_2} = \emph{\texttt{strike}}(\pi_x \mid \widetilde{T_1})$:
\[
\begin{aligned}
...
\]
\end{definition}
\begin{definition}[Mutable] Let $\Gamma$ be an environment where $\Gamma(x) = \langle \widetilde{T} \rangle^1$ for some lifetime 1, and $w$ an lval where $w \triangleq \pi_x \mid x$. Then $\emph{\texttt{mut}}(\Gamma, w)$ is a partial function defined as $\emph{\texttt{mutable}}(\Gamma, \pi_x \mid \widetilde{T})$ that determines whether $w$ is mutable:
\[
\begin{aligned}
...
\inferrule*[right=T-Const]{
    ...
    \Gamma \vdash \langle v : T \rangle^1_\sigma \dashv \Gamma
}
...
\inferrule*[right=T-Copy]{
    ...
    \Gamma \vdash \langle \hat{w} : T \rangle^1_\sigma \dashv \Gamma
}
...
\inferrule*[right=T-Move]{
    ...
    \Gamma_1 \vdash \langle w : T \rangle^1_\sigma \dashv \Gamma_2
}
...
\inferrule*[right=(T-MutBorrow)]{
    ...
    \Gamma \vdash \langle \&\texttt{mut}\;w : \&\texttt{mut}\;w \rangle^1_\sigma \dashv \Gamma
}
...
\inferrule*[right=(T-ImmBorrow)]{
    ...
    \Gamma \vdash \langle \&w : \&w \rangle^1_\sigma \dashv \Gamma
}

\inferrule*[right=T-Box]{
    \Gamma_1 \vdash \langle t : T \rangle^1_\sigma \dashv \Gamma_2
}{
    \Gamma_1 \vdash \langle \texttt{box}\;t : \square T \rangle^1_\sigma \dashv \Gamma_2
}

\inferrule*[right=T-Seq]{
    \Gamma_1 \vdash \langle t_1 : T_1 \rangle^1_\sigma \dashv \Gamma_2 \\
    \dots \\
    \Gamma_n \vdash \langle t_n : T_n \rangle^1_\sigma \dashv \Gamma_{n+1}
}{
    \Gamma_1 \vdash \langle \overline{t} : \overline{T_n} \rangle^1_\sigma \dashv \Gamma_{n+1}
}

\inferrule*[right=T-block]{
    \Gamma_1 \vdash \langle \overline{t} : T \rangle^m_\sigma \dashv \Gamma_2 \\
    \Gamma_2 \vdash T \geq 1 \\
    \Gamma_3 = \texttt{drop}(\Gamma_2, m)
}{
    \Gamma_1 \vdash \langle \{\overline{t}\}^m : T \rangle^1_\sigma \dashv \Gamma_3
}
...
\inferrule*[right=T-Declare]{
    ...
    \Gamma_1 \vdash \langle \texttt{let}\;\texttt{mut}\; x = t : \epsilon \rangle^1_\sigma \dashv \Gamma_3
}

\inferrule*[right=T-Assign]{
    \Gamma_1 \vdash w : \langle \widetilde{T_1} \rangle^m \\
    \Gamma_1 \vdash \langle t : T_2 \rangle^1_\sigma \dashv \Gamma_2 \\
    \Gamma_2 \vdash \widetilde{T_1} \approx T_2 \\
    \Gamma_2 \vdash T_2 \geq m \\
    \Gamma_3 = \texttt{write}^0(\Gamma_2, w, T_2) \\
    \neg \texttt{writeProhibited}(\Gamma_3, w)
}{
    \Gamma_1 \vdash \langle w = t : \epsilon \rangle^1_\sigma \dashv \Gamma_3
}
\end{mathpar}
