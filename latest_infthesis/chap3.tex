...
The rules \textsc{Borrow (s1)} and \textsc{Borrow (s2)} show the evaluation of immutable and mutable borrow operations. On a given node $n$, to borrow a remotely owned reference from a different node $n'$, a remotely borrowed reference $\mathscr{l}^\circ_{@n'}$ is passed back to the node $n$ at the end of the evaluation.
