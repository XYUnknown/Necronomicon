...
If it is immutably borrowed, then the corresponding entry will be immutably borrowed from the table (line 16 --- line 24). If it is mutably borrowed, then the corresponding entry will be mutably borrowed from the table and updated after the execution (line 29 --- line 38). 
If an argument is a proxy that is not located at the caller, as shown in three \texttt{else}-cases, from line 12 to line 14, from line 25 to line 27, and from line 39 to line 41, then the proxy will be passed into the method without any additional modification.
