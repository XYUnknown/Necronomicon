...
\begin{figure}
    ...
    \cmid \quad &\&\texttt{mut}\; \bar{w} &\text{mutable borrow}\\
    \cmid \quad &\&\bar{w} &\text{immutable borrow}\\
...
\begin{definition}[Copy Types] A type $T$ has copy semantics, denoted by \emph{\texttt{copy}}($T$), when $T = \emph{\texttt{int}}$ or $T = \&\bar{w}$.
\end{definition}
...
\end{definition}
\begin{definition}[LVal Typing] An lval $w$ is said to be typed with respect to an environment $\Gamma$, denoted $\Gamma \vdash w : \langle \widetilde{T}$, according to the following rules:
\end{definition}
\vspace{-2.0em}
\begin{mathpar}
...
\inferrule*[right=(T-LvBor)]{
  ...
  \Gamma \vdash \overline{u} : \langle \overline{T} \rangle^m
}{
